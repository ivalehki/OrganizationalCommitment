---
title: "ANALIZA"
output: html_document
date: "2025-11-30"
editor_options: 
  chunk_output_type: console
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

## Učitavanje rezultata ankete

Funkcija koja mi označava mjesto u podacima za koje vrijedi filter koji unesem: Anketa[which(anketa$Spol) == 'a'), 'Spol'] - dakle, prvi dio uglate zagrade označava redove u kojima je upisana vrijednost a, a drugi dio uglate zagrade je označena kolona koju pretražujemo.
```{r }
library(readxl)
library(tidyverse)
library(patchwork)
Anketa <- read_excel("ANKETA_ODGOVORI.xlsx")



```

#Preimenovanje vrijednosti u kolonama
```{r}
Anketa[which(Anketa$Spol == 'a'), 'Spol'] = 'Muško'
Anketa[which(Anketa$Spol == 'b'), 'Spol'] = 'Žensko'
Anketa[which(Anketa$Dob == 'a'), 'Dob'] = '<=30'
Anketa[which(Anketa$Dob == 'b'), 'Dob'] = '31-40'
Anketa[which(Anketa$Dob == 'c'), 'Dob'] = '41-50'
Anketa[which(Anketa$Dob == 'd'), 'Dob'] = '>=51'
Anketa[which(Anketa$RadniStaž == 'a'), 'RadniStaž'] = '<=5'
Anketa[which(Anketa$RadniStaž == 'b'), 'RadniStaž'] = '6-10'
Anketa[which(Anketa$RadniStaž == 'c'), 'RadniStaž'] = '11-20'
Anketa[which(Anketa$RadniStaž == 'd'), 'RadniStaž'] = '>=21'
Anketa[which(Anketa$BračniStatus == 'a'), 'BračniStatus'] = 'Neoženjen/Neudata'
Anketa[which(Anketa$BračniStatus == 'b'), 'BračniStatus'] = 'Oženjen/Udata/Izvanbračna zajednica'
Anketa[which(Anketa$BračniStatus == 'c'), 'BračniStatus'] = 'Razveden/a'
Anketa[which(Anketa$BračniStatus == 'd'), 'BračniStatus'] = 'Udovac/Udovica'
Anketa[which(Anketa$Obrazovanje == 'a'), 'Obrazovanje'] = 'NKV'
Anketa[which(Anketa$Obrazovanje == 'b'), 'Obrazovanje'] = 'KV'
Anketa[which(Anketa$Obrazovanje == 'c'), 'Obrazovanje'] = 'SSS'
Anketa[which(Anketa$Obrazovanje == 'd'), 'Obrazovanje'] = 'VKV'
Anketa[which(Anketa$Obrazovanje == 'e'), 'Obrazovanje'] = 'VŠS'
Anketa[which(Anketa$Obrazovanje == 'f'), 'Obrazovanje'] = 'VSS'
Anketa[which(Anketa$Obrazovanje == 'g'), 'Obrazovanje'] = 'MR/DR'
Anketa[which(Anketa$RadnoMjesto == 'a'), 'RadnoMjesto'] = 'Izvršno'
Anketa[which(Anketa$RadnoMjesto == 'b'), 'RadnoMjesto'] = 'Rukovodeće'
Anketa[which(Anketa$VrstaPosla == 'c'), 'VrstaPosla'] = 'Određeno'
Anketa[which(Anketa$VrstaPosla == 'd'), 'VrstaPosla'] = 'Neodređeno'

```

KOLOR PALETA SE ZOVE WATER & RUST
#151F30 (TAMNO PLAVA), #103778 (SVJETLIJE PLAVA), #0593A2 (SVIJETLO PLAVA), #FF4F16 (NARANČASTA), #E3371E (CIGLENO CRVENA) 



# DISTRIBUCIJA SPOLA ISPITANIKA
```{r}
P1 = ggplot(Anketa, aes(x=Spol, fill = Spol))+
  geom_bar(stat='count') +
  scale_fill_manual(values = c('Muško' = '#103778', 'Žensko' = '#FF7A48')) +
  theme_minimal()+
  ylab('Broj ispitanika')

ggsave('Tablica1.png', P1)
```

### DISTRIBUCIJA GODINA ISPITANIKA
```{r, echo=FALSE}
P2 = ggplot(Anketa, aes(x=Dob)) +
  geom_bar(stat='count', fill = '#151F30') +
  theme_minimal() + 
  ylab('Broj ispitanika')

ggsave('Tablica2.png', P2 )

```
### DISTRIBUCIJA RADNOG STAŽA ISPITANIKA
```{r}
P3 = ggplot(Anketa, aes(x=RadniStaž)) +
  geom_bar(stat='count', fill = '#151F30') +
  theme_minimal() +
  ylab('Broj ispitanika') + 
  xlab('Radni Staž')

P3
ggsave('Tablica3.png', P3)
```

### DISTIRBUCIJA OBRAZOVANJA ISPITANIKA

```{r}
P4 = ggplot(Anketa, aes(x=Obrazovanje)) +
  geom_bar(stat='count', fill = '#151F30') +
  theme_minimal() +
  ylab('Broj ispitanika')

P4

```

### DISTRIBUCIJA DEMOGRAFSKIH POKAZATELJA ISPITANIKA

```{r}
P1 + P2 + P3 + P4
ggsave('Tablica_demografija_part1.png', P1 + P2 + P3 + P4 )
```



### DISTRIBUCIJA BRAČNOG STATUSA ISPITANIKA
```{r}
P5 = ggplot(Anketa, aes(x=BračniStatus, fill=BračniStatus)) + 
  geom_bar(stat = 'count') +
  theme_minimal() +
  ylab('Broj ispitanika') + 
  xlab('Bračni status') + 
  scale_fill_manual(values = c('Neoženjen/Neudata' = '#151F30', 'Oženjen/Udata/Izvanbračna zajednica' = '#103778', 'Razveden/a' = '#0593A2', 'Udovac/Udovica' = '#FF7A48'))

P5
ggsave('Tablica4.png', P5)
```

### DISTRIBUCIJA RADNOG MJESTA ISPITANIKA
```{r}
P6 = ggplot (Anketa, aes(x=RadnoMjesto)) +
  geom_bar(stat = 'count', fill = '#151F30') + 
  theme_minimal() +
  ylab('Broj ispitanika') +
  xlab('Radno mjesto')

P6
```

### DISTRIBUCIJA VRSTE POSLA ISPITANIKA

```{r}
P7 = ggplot(Anketa, aes(x=VrstaPosla)) +
  geom_bar(stat = 'count', fill = '#151F30') +
  theme_minimal() +
  ylab('Broj ispitanika') +
  xlab('Vrsta posla') 

P7
```

### DISTRIBUCIJA DEMOGRAFSKIH POKAZATELJA ISPITANIKA PART 2

```{r}
P5 + P6 + P7
ggsave('Tablica_demografija_part2.png', P5 / (P6 + P7), height = 8, width = 12)
```

### ANALIZA INDIVIDUALNIH ODGOVORA ISPITANIKA (PROSJEK/MEAN, SETOVI PITANJA - AFEKTIVNE KOMPONENTE)
```{r}
ggplot(Anketa)+
 stat_summary( aes(x ='1. Bio/la bih jako sretan/na kada bih proveo/la ostatak karijere u tvrtki u kojoj radim.', y=`8`), fun = "mean", geom = "bar", fill = '#103778' ) +
 stat_summary( aes(x ='2. Zaista osjećam da su problemi koje ima moja tvrtka ujedno i moji problemi.', y=`9`), fun = "mean", geom = "bar", fill = '#103778' ) +
 stat_summary( aes(x ='3. Osjećam se kao „dio obitelji“ u ovoj tvrtki.', y=`10`), fun = "mean", geom = "bar", fill = '#103778' ) +
 stat_summary( aes(x ='4. Osjećam emocionalnu povezanost prema svojoj tvrtki.', y=`11`), fun = "mean", geom = "bar", fill = '#103778' ) +
 stat_summary( aes(x ='5. Ova tvrtka ima veliko osobno značenje za mene.', y=`12`), fun = "mean", geom = "bar", fill = '#103778' ) +
 stat_summary( aes(x ='6. Imam snažan osjećaj da pripadam tvrtki u kojoj radim.', y=`13`), fun = "mean", geom = "bar", fill = '#103778') +
 theme_minimal() + 
 coord_flip()+
 xlab("") + 
 ylab("Prosječna ocjena")
 

```


###### ANALIZA INDIVIDUALNIH ODGOVORA ISPITANIKA (PROSJEK/MEAN, SETOVI PITANJA - KONTINUIRANE KOMPONENTE)

```{r}
ggplot(Anketa)+
 stat_summary( aes(x ='7. Bilo bi mi jako teško (nezgodno) sada napustiti posao, čak i kada bi mi to bila želja. ', y=`14`), fun = "mean", geom = "bar", fill = '#FF7A48' ) +
 stat_summary( aes(x ='8. Previše bi mi se toga u životu poremetilo kada bih otišao iz svoje tvrtke.', y=`15`), fun = "mean", geom = "bar", fill = '#FF7A48' ) +
 stat_summary( aes(x ='9. Trenutačno je razlog moga ostanka u tvrtki stvar nužnosti koliko i želje.', y=`16`), fun = "mean", geom = "bar", fill = '#FF7A48' ) +
 stat_summary( aes(x ='10. Smatram da imam premalo drugih mogućnosti da bih razmatrao/la o odlasku iz tvrtke.', y=`17`), fun = "mean", geom = "bar", fill = '#FF7A48' ) +
 stat_summary( aes(x ='11. Jedna od nekoliko negativnih posljedica napuštanja posla u tvrtki će biti mala mogućnost za pronaći posao negdje drugdje.', y=`18`), fun = "mean", geom = "bar", fill = '#FF7A48' ) +
 stat_summary( aes(x ='12. Jedan od glavnih razloga što ću nastaviti raditi u ovoj tvrtki je taj što bi odlazak zahtijevao previše osobnog žrtvovanja.', y=`19`), fun = "mean", geom = "bar", fill = '#FF7A48') +
 theme_minimal() + 
 coord_flip()+
 xlab("") + 
 ylab("Prosječna ocjena")
```

###### ANALIZA INDIVIDUALNIH ODGOVORA ISPITANIKA (PROSJEK/MEAN, SETOVI PITANJA - NORMATIVNE KOMPONENTE)
```{r}
ggplot(Anketa)+
 stat_summary( aes(x ='13. Osjećam obavezu da ostanem u svojoj tvrtki.', y=`20`), fun = "mean", geom = "bar", fill = '#0593A2' ) +
 stat_summary( aes(x ='14. Čak i kada bih imao/la određenu korist od odlaska, osjećam da to ne bi bilo u redu.', y=`21`), fun = "mean", geom = "bar", fill = '#0593A2' ) +
 stat_summary( aes(x ='15. Osjećati ću se krivo ako sada napustim svoju tvrtku.', y=`22`), fun = "mean", geom = "bar", fill = '#0593A2' ) +
 stat_summary( aes(x ='16. Ova organizacija zaslužuje moju lojalnost.', y=`23`), fun = "mean", geom = "bar", fill = '#0593A2' ) +
 stat_summary( aes(x ='17. Ne bih mogao/la napustiti svoju tvrtku sada, jer imam osjećaj obveze (dugovanja) prema njoj.', y=`24`), fun = "mean", geom = "bar", fill = '#0593A2' ) +
 stat_summary( aes(x ='18. Mnogo dugujem ovoj tvrtki.', y=`25`), fun = "mean", geom = "bar", fill = '#0593A2') +
 theme_minimal() + 
 coord_flip()+
 xlab("") + 
 ylab("Prosječna ocjena")
```


### RAČUNANJE PROSJEČNIH VRIJEDNOSTI PO PITANJIMA IZ SETA AFEKTIVNE PREDANOSTI
```{r}
cat( "1." ,mean(Anketa$`8`))
cat( "2." ,mean(Anketa$`9`) )
cat( "3." ,mean(Anketa$`10`))
cat( "4." ,mean(Anketa$`11`))
cat( "5." ,mean(Anketa$`12`))
cat( "6." ,mean(Anketa$`13`))
```

#KONKATINEJT!!! #CONCATINATE!!! >> LINK THINGS, CHAIN - SPOJITI

### RAČUNANJE PROSJEČNIH VRIJEDNOSTI PO PITANJIMA IZ SETA KONTINUIRANE PREDANOSTI
```{r}
cat( "7." ,mean(Anketa$`14`))
cat( "8." ,mean(Anketa$`15`) )
cat( "9." ,mean(Anketa$`16`))
cat( "10." ,mean(Anketa$`17`))
cat( "11." ,mean(Anketa$`18`))
cat( "12." ,mean(Anketa$`19`))
```

### RAČUNANJE PROSJEČNIH VRIJEDNOSTI PO PITANJIMA IZ SETA NORMATIVNE PREDANOSTI
```{r}
cat( "13." ,mean(Anketa$`20`))
cat( "14." ,mean(Anketa$`21`))
cat( "15." ,mean(Anketa$`22`))
cat( "16." ,mean(Anketa$`23`))
cat( "17." ,mean(Anketa$`24`))
cat( "18." ,mean(Anketa$`25`))
```

### RAČUNANJE PROSJEČNOG STUPNJA SLAGANJA ISPITANIKA PO KOMPONENTAMA (AF, NORM, KONT)  
```{r}
Anketa[,'AF_rez'] = rowMeans(Anketa[,c(8:13)])
Anketa[, 'Kont_rez']= rowMeans(Anketa[, c(14:19)])
Anketa[, 'Norm_rez']= rowMeans(Anketa[,c(20:25)])
```

### USPOREDBA PROSJEČNIH STUPNJEVA SLAGANJA PO DEMOGRAFIJI

```{r}
library (ggpubr) 
hist(Anketa$AF_rez)

ggplot(Anketa, aes(x=Spol, y=AF_rez, fill=Spol))+
  geom_boxplot()+
  stat_compare_means(method = 't.test')+
  theme_minimal()+
ylab('Stupanj afektivne predanosti')+
scale_fill_manual(values = c('Muško'='#151F30', 'Žensko'='#FF7A48'))
```

### USPOREDBA PROSJEČNIH STUPNJEVA SLAGANJA PREMA RADNOM MJESTU

```{r}
library(ggpubr)
hist(Anketa$RadnoMjesto)

ggplot(Anketa, aes(x=RadnoMjesto, y=Kont_rez, fill =RadnoMjesto))+
  geom_boxplot()+
  geom_point()+
  stat_compare_means(method = 't.test')+
  theme_minimal()+
ylab('Stupanj afektivne predanosti')+
scale_fill_manual(values = c('Izvršno'='#103778', 'Rukovodeće'='#0593A2'))

```


### PROSJECI SVIH SETOVA TVRDNJI PO KOMPONENTAMA
```{r}
mean(c(Anketa$`20`, Anketa$`21`, Anketa$`22`, Anketa$`23`, Anketa$`24`, Anketa$`25`))
```

### PROSJECI SVE TRI OCJENE KOMPONENTI PREDANOSTI
```{r}
mean(Anketa$AF_rez)
mean(Anketa$Kont_rez)
mean(Anketa$Norm_rez)
```

### HISTOGRAMI OCJENA KOMPONENTI PREDANOSTI
```{r}
P1=ggplot(Anketa, aes(x=AF_rez)) +
  geom_histogram(bins=10, fill = '#103778', color = "black") +
  geom_vline(xintercept = mean(Anketa$AF_rez),linetype="dashed", linewidth = 1) +  
  theme_minimal() +
  ylab('Broj ispitanika') +
  xlab('Afektivna ocjena') 

P2=ggplot(Anketa, aes(x=Kont_rez)) +
  geom_histogram(bins=10, fill = '#103778', color = "black") +
  geom_vline(xintercept = mean(Anketa$AF_rez),linetype="dashed", linewidth = 1) +
  theme_minimal() +
  ylab('Broj ispitanika') +
  xlab('Kontinuirana ocjena') 

P3=ggplot(Anketa, aes(x=Norm_rez)) +
  geom_histogram(bins=10, fill = '#103778', color = "black") +
  geom_vline(xintercept = mean(Anketa$AF_rez),linetype="dashed", linewidth = 1) +
  theme_minimal() +
  ylab('Broj ispitanika') +
  xlab('Normativna ocjena') 

P1+P2+P3

```



### PRIKAZ OCJENA PO RADNOM STAŽU
```{r}
library(ggpubr)

Anketa$RadniStažMerge = ifelse(Anketa$RadniStaž %in% c("<=5", "6-10"), "<10", ">10") 

P1 = ggplot(Anketa, aes(x=RadniStažMerge, y=AF_rez, fill = RadniStažMerge)) + 
  geom_boxplot() + 
  geom_point() +
  theme_minimal() + 
  xlab("Radni Staž") +
  ylab("Afektivna ocjena") +
  scale_fill_manual(values = c('<10' = '#103778', '>10' = '#0593A2')) +
  stat_compare_means(method = "t.test")+
  theme(legend.position = "none")

P2 = ggplot(Anketa, aes(x=RadniStažMerge, y=Kont_rez, fill = RadniStažMerge)) +
  geom_boxplot() +
  geom_point() +
  theme_minimal() +
  xlab("Radni Staž") + 
  ylab("Kontinuirana ocjena") +
  scale_fill_manual(values = c('<10' = '#103778', '>10' = '#0593A2')) +
  stat_compare_means(method = "t.test")+
  theme(legend.position = "none")

P3 = ggplot(Anketa, aes(x=RadniStažMerge, y=Norm_rez, fill = RadniStažMerge)) +
  geom_boxplot() +
  geom_point() +
  theme_minimal() +
  xlab("Radni Staž") +
  ylab("Normativna ocjena") + 
  scale_fill_manual(values = c('<10' = '#103778', '>10' = '#0593A2')) +
  stat_compare_means(method = "t.test")+
  theme(legend.position = "none")

P1 + P2 + P3

```

### PRIKAZ OCJENA PO SPOLU

```{r}
P1 = ggplot(Anketa, aes(x=Spol, y=AF_rez, fill = Spol)) + 
  geom_boxplot() + 
  geom_point() +
  theme_minimal() + 
  xlab("Spol") +
  ylab("Afektivna ocjena") +
  scale_fill_manual(values = c('Muško' = '#103778', 'Žensko' = '#0593A2')) +
  stat_compare_means(method = "t.test")+
  theme(legend.position = "none")

P2 = ggplot(Anketa, aes(x=Spol, y=Kont_rez, fill = Spol)) +
  geom_boxplot() +
  geom_point() +
  theme_minimal() +
  xlab("Spol") + 
  ylab("Kontinuirana ocjena") +
  scale_fill_manual(values = c('Muško' = '#103778', 'Žensko' = '#0593A2')) +
  stat_compare_means(method = "t.test")+
  theme(legend.position = "none")

P3 = ggplot(Anketa, aes(x=Spol, y=Norm_rez, fill = Spol)) +
  geom_boxplot() +
  geom_point() +
  theme_minimal() +
  xlab("Spol") +
  ylab("Normativna ocjena") + 
  scale_fill_manual(values = c('Muško' = '#103778', 'Žensko' = '#0593A2')) +
  stat_compare_means(method = "t.test")+
  theme(legend.position = "none")


P1 + P2 + P3

```

### PRIKAZ OCJENA PO DOBI

```{r}
Anketa$DobMerge = ifelse(Anketa$Dob %in% c('<=30', '31-40'), "<=40", ">40" )

P1 = ggplot(Anketa, aes(x=DobMerge, y=AF_rez, fill = DobMerge)) + 
  geom_boxplot() + 
  geom_point() +
  theme_minimal() + 
  xlab("Dob") +
  ylab("Afektivna ocjena") +
  scale_fill_manual(values = c('<=40' = '#103778', '>40' = '#0593A2')) +
  stat_compare_means(method = "t.test") +
  theme(legend.position = "none")

P2 = ggplot(Anketa, aes(x=DobMerge, y=Kont_rez, fill = DobMerge)) +
  geom_boxplot() +
  geom_point() +
  theme_minimal() +
  xlab("Dob") + 
  ylab("Kontinuirana ocjena") +
  scale_fill_manual(values = c('<=40' = '#103778', '>40' = '#0593A2')) +
  stat_compare_means(method = "t.test") +
  theme(legend.position = "none")

P3 = ggplot(Anketa, aes(x=DobMerge, y=Norm_rez, fill = DobMerge)) +
  geom_boxplot() +
  geom_point() +
  theme_minimal() +
  xlab("Dob") +
  ylab("Normativna ocjena") + 
  scale_fill_manual(values = c('<=40' = '#103778', '>40' = '#0593A2')) +
  stat_compare_means(method = "t.test") +
  theme(legend.position = "none")


P1 + P2 + P3
```

### PRIKAZ OCJENA PO BRAČNOM STATUSU

```{r}
Anketa$BračniStatusMerge = ifelse(Anketa$BračniStatus %in% c('Oženjen/Udata/Izvanbračna zajednica'),"Zauzeti", "Slobodni" )

P1 = ggplot(Anketa, aes(x=BračniStatusMerge, y=AF_rez, fill = BračniStatusMerge)) + 
  geom_boxplot() + 
  geom_point() +
  theme_minimal() + 
  xlab("Bračni status") +
  ylab("Afektivna ocjena") +
  scale_fill_manual(values = c('Zauzeti' = '#103778', 'Slobodni' = '#0593A2')) +
  stat_compare_means(method = "t.test") +
  theme(legend.position = "none")


P2 = ggplot(Anketa, aes(x=BračniStatusMerge, y=Kont_rez, fill = BračniStatusMerge)) + 
  geom_boxplot() + 
  geom_point() +
  theme_minimal() + 
  xlab("Bračni status") +
  ylab("Kontinuirana ocjena") +
  scale_fill_manual(values = c('Zauzeti' = '#103778', 'Slobodni' = '#0593A2')) +
  stat_compare_means(method = "t.test") +
  theme(legend.position = "none")

P3 = ggplot(Anketa, aes(x=BračniStatusMerge, y=Norm_rez, fill = BračniStatusMerge)) + 
  geom_boxplot() + 
  geom_point() +
  theme_minimal() + 
  xlab("Bračni status") +
  ylab("Normativna ocjena") +
  scale_fill_manual(values = c('Zauzeti' = '#103778', 'Slobodni' = '#0593A2')) +
  stat_compare_means(method = "t.test") +
  theme(legend.position = "none")


P1 + P2 + P3
```

### PRIKAZ OCJENA PO OBRAZOVANJU

```{r}
Anketa$ObrazovanjeMerge = ifelse(Anketa$Obrazovanje %in% c('KV', 'SSS'),"Niži S. O.", "Viši S. O." )

P1 = ggplot(Anketa, aes(x=ObrazovanjeMerge, y=AF_rez, fill = ObrazovanjeMerge)) + 
  geom_boxplot() + 
  geom_point() +
  theme_minimal() + 
  xlab("Stupanj obrazovanja") +
  ylab("Afektivna ocjena") +
  scale_fill_manual(values = c('Niži S. O.' = '#103778', 'Viši S. O.' = '#0593A2')) +
  stat_compare_means(method = "t.test") +
  theme(legend.position = "none")


P2 = ggplot(Anketa, aes(x=ObrazovanjeMerge, y=Kont_rez, fill = ObrazovanjeMerge)) + 
  geom_boxplot() + 
  geom_point() +
  theme_minimal() + 
  xlab("Stupanj obrazovanja") +
  ylab("Kontinuirana ocjena") +
  scale_fill_manual(values = c('Niži S. O.' = '#103778', 'Viši S. O.' = '#0593A2')) +
  stat_compare_means(method = "t.test") +
  theme(legend.position = "none")

P3 = ggplot(Anketa, aes(x=ObrazovanjeMerge, y=Norm_rez, fill = ObrazovanjeMerge)) + 
  geom_boxplot() + 
  geom_point() +
  theme_minimal() + 
  xlab("Stupanj obrazovanja") +
  ylab("Normativna ocjena") +
  scale_fill_manual(values = c('Niži S. O.' = '#103778', 'Viši S. O.' = '#0593A2')) +
  stat_compare_means(method = "t.test") +
  theme(legend.position = "none")


P1 + P2 + P3
```

```{r}
Anketa$ObrazovanjeMerge = ifelse(Anketa$Obrazovanje %in% c('KV', 'SSS'),"Niži S. O.", "Viši S. O." )

Anketa$BračniStatusMerge = ifelse(Anketa$BračniStatus %in% c('Oženjen/Udata/Izvanbračna zajednica'),"Zauzeti", "Slobodni" )

Anketa$DobMerge = ifelse(Anketa$Dob %in% c('<=30', '31-40'), "<=40", ">40" )

Anketa$RadniStažMerge = ifelse(Anketa$RadniStaž %in% c("<=5", "6-10"), "<10", ">10")

### PRIKAZ 12: T-test utjecaj grupiranih pokazatelja bračnog statusa i radnog staža na kontinuiranu predanost
ggplot(Anketa, aes(x=BračniStatusMerge, y=Kont_rez, fill = RadniStažMerge)) + 
  geom_boxplot() + 
  geom_point() +
  theme_minimal() + 
  xlab("Bračni status") +
  ylab("Kontinuirana ocjena") +
  scale_fill_manual(values = c('<10' = '#103778', '>10' = '#0593A2')) +
  stat_compare_means(method = "t.test") +
  guides(fill = guide_legend(title = "Radni staž"))


### PRIKAZ 13: T-test utjecaj grupiranih pokazatelja bračnog statusa i radnog staža na normativnu predanost
ggplot(Anketa, aes(x=BračniStatusMerge, y=Norm_rez, fill = RadniStažMerge)) + 
  geom_boxplot() + 
  geom_point() +
  theme_minimal() + 
  xlab("Bračni status") +
  ylab("Normativna ocjena") +
  scale_fill_manual(values = c('<10' = '#103778', '>10' = '#0593A2')) +
  stat_compare_means(method = "t.test") +
  guides(fill = guide_legend(title = "Radni staž"))


### PRIKAZ 14: T-test utjecaj grupiranih pokazatelja obrazovanja i radnog staža na normativnu predanost
ggplot(Anketa, aes(x=ObrazovanjeMerge, y=Norm_rez, fill = RadniStažMerge)) + 
  geom_boxplot() + 
  geom_point() +
  theme_minimal() + 
  xlab("Obrazovanje") +
  ylab("Normativna ocjena") +
  scale_fill_manual(values = c('<10' = '#103778', '>10' = '#0593A2')) +
  stat_compare_means(method = "t.test") +
  guides(fill = guide_legend(title = "Radni staž"))


Anketa$DobMerge = ifelse(Anketa$Dob %in% c('>=51'), ">=51", "<51" )

### PRIKAZ 15: T-test utjecaj grupiranih pokazatelja dobi i bračnog statusa na af predanost
ggplot(Anketa, aes(x=BračniStatusMerge, y=AF_rez, fill = DobMerge)) + 
  geom_boxplot() + 
  geom_point() +
  theme_minimal() + 
  xlab("Bračni status") +
  ylab("Afektivna ocjena") +
  scale_fill_manual(values = c('>=51' = '#103778', '<51' = '#0593A2')) +
  stat_compare_means(method = "t.test") +
  guides(fill = guide_legend(title = "Dob"))

### PRIKAZ 16: T-test utjecaj grupiranih pokazatelja dobi i bračnog statusa na normativnu predanost
ggplot(Anketa, aes(x=BračniStatusMerge, y=Norm_rez, fill = DobMerge)) + 
  geom_boxplot() + 
  geom_point() +
  theme_minimal() + 
  xlab("Bračni status") +
  ylab("Normativna ocjena") +
  scale_fill_manual(values = c('>=51' = '#103778', '<51' = '#0593A2')) +
  stat_compare_means(method = "t.test") +
  guides(fill = guide_legend(title = "Dob"))

### PRIKAZ 17: T-test utjecaj grupiranih pokazatelja spola i radnog staža na afektivnu predanost
ggplot(Anketa, aes(x=RadniStažMerge, y=AF_rez, fill = Spol)) + 
  geom_boxplot() + 
  geom_point() +
  theme_minimal() + 
  xlab("Radni staž") +
  ylab("Afektivna ocjena") +
  scale_fill_manual(values = c('Muško' = '#103778', 'Žensko' = '#0593A2')) +
  stat_compare_means(method = "t.test") +
  guides(fill = guide_legend(title = "Spol"))


### PRIKAZ 18: 


```








